-- Fix: Change v_work_mins to NUMERIC to prevent rounding errors.
-- Fix: Exclude 'overtime_reduction' from project hours calculation in get_monthly_stats (consistency with lifetime stats).
...
DECLARE
    ...
    v_work_mins NUMERIC; -- Changed from INT to NUMERIC
...
BEGIN
    ...
    WHERE user_id = p_user_id
        ...
        AND date <= v_end_date
        -- Matches Frontend "projectHours" filter AND NOW includes overtime_reduction exclusion
        AND type NOT IN ('break', 'vacation', 'sick', 'holiday', 'special_holiday', 'sick_child', 'sick_pay', 'unpaid', 'overtime_reduction')
